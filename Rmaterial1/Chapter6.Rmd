...
`density(x)` estimates the density of `x` by counting how many observations fall in a little window around each point, and then smoothing  
    ...
    <small>Take 36-402 in the spring for much more</small>
...
```
<small>(will see how to do this for real later in class)</small>
---
...
- _Assume_ that for reach $i$, $g_i(x_{1:n}) \rightarrow g_i(\theta^*)$
- _Define_
