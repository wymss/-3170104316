...
1. The Iowa data set iowa.csv is a toy example that summarises the yield of wheat (bushels per acre) for the state of Iowa between 1930-1962. In addition to yield, year, rainfall and temperature were recorded as the main predictors of yield.
    ...
    b. How many rows and columns does `iowa.df` have? 31行 1列
    c. What are the names of the columns of `iowa.df`? colname = ["Year"  "Rain0" "Temp1" "Rain1" "Temp2" "Rain2" "Temp3" "Rain3" "Temp4" "Yield"]
    d. What is the value of row 5, column 7 of `iowa.df`? 值是79.7
    e. Display the second row of `iowa.df` in its entirety. row2 = [1931 14.76  57.5  3.83    75  2.72  77.2   3.3  72.6 32.9]
```{r}
...
3. Working with functions and operators.
    ...
    rep(1:3, times=3)的结果是[ 1 2 3 1 2 3 1 2 3]，rep(1:3,each = 3)的结果是[1 1 1 2 2 2 3 3 3]。一个重复三次，一个是一次重复三次
```{r}
a1 <- seq(1,10000,372)
...
```{r}
aissport <- unique(as.character(ais$sport))
b <- list()
for (i in 1:length(aissport)){
  b <- c(b,list(c(0,0)))
}
names(b) <- aissport
for (i in 1:dim(ais)[1]){
  if (ais[i,12] == "f"){
    b[[ais[i,13]]][1] <- b[[ais[i,13]]][1] + 1
  }
  if (ais[i,12] == "m"){
    b[[ais[i,13]]][2] <- b[[ais[i,13]]][2] + 1
  }
}
d <- c()
for(i in 1:length(aissport)){
...
Playgreen            217   657
```{r}
a <- data.frame(elevation = c(217,254,248,254,253,227,178,207, 217),area = c(24387,5374,4624,2247,1353,1223,1151,755,657),row.names = c("Winnipeg","Winnipegosis","Manitoba ","SouthernIndian","Cedar","Island","Gods ","Cross","Playgreen  "))
print(a)
```
